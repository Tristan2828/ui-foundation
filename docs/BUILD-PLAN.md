> [!Important]
> **Historical document — the build this plan describes is finished.** It is
> kept as the record of why things are the way they are (the Decision Ledger,
> the phase rationale). It is **not** read every session any more, and parts of
> it describe things that were planned but never built (the optional hook tier,
> `phase-verifier`, `/phase`, `.codex/hooks.json`) or later replaced
> (kitchen-sink, the `openapi.yaml` freeze, the `check-phase-N.sh` scripts). The
> current architecture is [`ARCHITECTURE.md`](ARCHITECTURE.md); current status is
> [`STATUS.md`](STATUS.md).
...
# AGENTS.md and the Entity Playbook
---

Both used to be embedded here as templates. The copies drifted from the real
files (the playbook copy never gained Step 0; the AGENTS.md copy claimed a Stop
hook that was never built), so they were removed in audit Phase E. The only
copies are `AGENTS.md` and `docs/add-an-entity.md`.
