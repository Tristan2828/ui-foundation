#!/usr/bin/env bash
# DATABASE_SSL against a real hosted Postgres (Phase 12 — docs/cloud-postgres.md).
# Additive, not a replacement: docker-compose.yml stays the default local
# path, proven at the end of this script by chaining onto check-backend-postgres.sh
...
# same "a human must actually do this part" shape check-backend-postgres.sh's Docker
# step has. See docs/cloud-postgres.md.
set -euo pipefail
cd "$(dirname "$0")/.."
REPO_ROOT="$(pwd)"

fail() { echo "check-cloud-postgres: $1" >&2; exit 1; }
...
# exported there, not at this script's top level — check-backend-postgres.sh runs
...
echo "check-cloud-postgres: alembic upgrade head against CLOUD_DATABASE_URL"
...
echo "check-cloud-postgres: backend/scripts/verify.sh against CLOUD_DATABASE_URL"
...
echo "check-cloud-postgres: starting uvicorn on :8001 against CLOUD_DATABASE_URL"
# Port 8001, not 8000 — check-backend-postgres.sh runs later in this same script and
# uses :8000 itself; running both at once on the same port would collide.
(set_cloud_env && cd backend && exec "../$PY" -m uvicorn app.main:app --port 8001 >"$REPO_ROOT/logs/cloud-postgres-uvicorn.log" 2>&1) &
UVICORN_PID=$!
...
echo "check-cloud-postgres: confirming the local Docker Compose path still works unmodified"
scripts/check-backend-postgres.sh || fail "check-backend-postgres.sh failed — the cloud-Postgres addition must not have broken the local one"

echo "check-cloud-postgres: PASS"
