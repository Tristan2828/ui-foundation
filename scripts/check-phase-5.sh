...
# Badge must be visible on /kitchen-sink so axe and the screenshot baseline
# actually exercise the fixed variant (Phase 3/4's contrast fixes were never
# independently verified in dark mode precisely because Badge had no section).
grep -q 'name="badge"' src/routes/kitchen-sink.tsx || fail 'kitchen-sink.tsx has no <Section name="badge">'
grep -q "'badge'" e2e/shell.spec.ts || fail "e2e/shell.spec.ts's KITCHEN_SECTIONS does not include 'badge'"

SNAPSHOT_DIR="e2e/shell.spec.ts-snapshots"
[ -f "$SNAPSHOT_DIR/kitchen-badge-dark-chromium-win32.png" ] ||
  fail "no committed win32 dark-mode baseline for the badge section"
git ls-files --error-unmatch "$SNAPSHOT_DIR/kitchen-badge-dark-chromium-win32.png" >/dev/null 2>&1 ||
...
grep -q "zero axe violations in dark mode" e2e/shell.spec.ts ||
  fail "e2e/shell.spec.ts has no dark-mode axe check"
